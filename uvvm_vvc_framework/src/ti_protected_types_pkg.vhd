...
package ti_protected_types_pkg is
  ...
  type t_vvc_activity is protected
    ...
    procedure priv_register_vvc_proc (
      signal   vvc_idx_out   : out integer;
      constant name          : in string;
      constant instance      : in natural;
      constant channel       : in t_channel := NA;
      constant num_executors : in positive  := 1
...
package body ti_protected_types_pkg is
  ...
  type t_vvc_activity is protected body
    ...
    type t_vvc_state is record
      activity              : t_activity_array(0 to 255); -- Each executor inside a VVC has its own activity status.
      ...
      pending_cmd_idx_list  : integer_vector(0 to 0);   -- Pending command indexes running in the VVC executors. Not used in VVCs with a single executor.
    end record;
    ...
    procedure priv_register_vvc_proc (
      ...
      constant instance      : in natural;
      constant channel       : in t_channel := NA;
      constant num_executors : in positive  := 1) is
    begin
      ...
      if priv_get_vvc_idx(name, instance, channel) = -1 then
        ...
        priv_registered_vvc(priv_last_registered_vvc_idx).vvc_state.activity                  := (others => INACTIVE);
      ...
      -- Return index
      vvc_idx_out <= priv_last_registered_vvc_idx;
    end procedure;
